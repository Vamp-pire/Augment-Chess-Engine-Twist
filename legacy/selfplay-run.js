...
// NOTE: this only takes effect on a fresh `node selfplay-run.js` invocation
// -- an already-running process has this constant baked in from startup and
// won't pick up the change.
const MAX_PLIES = 300;
const OUT_FILE = process.env.SELFPLAY_OUT_FILE
  ? path.join(__dirname, process.env.SELFPLAY_OUT_FILE)
  : path.join(__dirname, "selfplay-data.jsonl");
...
function runGame() {
  ...
  return new Promise((resolve) => {
    const worker = new Worker(path.join(__dirname, "selfplay-worker.js"), {
...
async function main() {
  ...
  const t0 = Date.now();
  // 30-minute progress heartbeat -- added so a long run can be checked on
  // partway through (games/sec trend, error rate, unfinished ratio) instead
  // of only finding out something went wrong after the whole budget is
  // already spent.
