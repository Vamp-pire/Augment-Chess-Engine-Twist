...
- [x] 깊이 확인 대전 완료(깊이 3 대 1 +61.4 Elo LOS 99.3%, 깊이 2 대 1 +30.5: 탐색은 실력을 올림, 결과는 `ExperimentNote.md` 10번). 남은 후속: 깊이 고정 모델 대 핸드코딩 대전, 엔진 속도로 깊이 확보. (원래 항목: run 36223518128, 36223543825, 결과 해석 기준은 `PLAN.md` "탐색 효과 확인과 다음 방향"): 시간 무제한에 가깝게 주고 깊이만 고정(예: `limits_a {"depth":1,"movetimeMs":60000}` 대 `{"depth":3,"movetimeMs":60000}`), 약 40판. 깊이 3이 확실히 이기면 탐색 정상(병목은 눈/정답), 아니면 탐색이나 게임 특성 문제. 배경: 기본 설정(0.3초)은 중반 국면에서 완료 깊이 0, 노드 7~12개.
