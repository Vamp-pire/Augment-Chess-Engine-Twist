...
const BETA_SUPABASE_AUTH_URL = "https://auth.augmentchess.org";
const BETA_SUPABASE_PUBLISHABLE_KEY = "sb_publishable_dhlcKKHD0OW8qApU2JlpbQ_lqqqbRcI";
const UUID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu;
const ACCOUNT_SESSION_PATH = "/api/session/account";
const PLAYER_SESSION_PATH = "/api/session";
const PLAYER_KEYS = Object.freeze([
  "accountId",
  "accountVerifiedUntilMs",
  "displayName",
  "expiresAtMs",
  "issuedAtMs",
  "playerId"
]);
const RESPONSE_KEYS = Object.freeze(["ok", "player", "protocolVersion"]);
let betaAuthClient = null;
let accountMutation = Promise.resolve(null);
const accountStatusListeners = /* @__PURE__ */ new Set();
function exactKeys(value, keys) {
  return Boolean(value && typeof value === "object" && !Array.isArray(value) && Object.keys(value).sort().join(",") === [...keys].sort().join(","));
}
function validPlayer(player) {
  return Boolean(exactKeys(player, PLAYER_KEYS) && UUID_PATTERN.test(String(player.playerId ?? "")) && (player.accountId === null || UUID_PATTERN.test(String(player.accountId ?? ""))) && player.accountId === null === (player.accountVerifiedUntilMs === null) && (player.accountVerifiedUntilMs === null || Number.isSafeInteger(player.accountVerifiedUntilMs)) && typeof player.displayName === "string" && player.displayName.length >= 1 && player.displayName.length <= 32 && Number.isSafeInteger(player.issuedAtMs) && Number.isSafeInteger(player.expiresAtMs) && player.expiresAtMs > player.issuedAtMs);
}
function validSessionResponse(body) {
  return Boolean(exactKeys(body, RESPONSE_KEYS) && body.ok === true && body.protocolVersion === 2 && validPlayer(body.player));
}
async function readJson(response) {
  ...
  if (!response.ok || !validSessionResponse(body)) {
    const error = new Error(String(body?.code || "BETA_ACCOUNT_SESSION_INVALID"));
    error.code = String(body?.code || "BETA_ACCOUNT_SESSION_INVALID");
    error.status = response.status;
    const retryAfterMs = Number(body?.retryAfterMs) || Number(response.headers.get("retry-after")) * 1e3;
    if (Number.isFinite(retryAfterMs) && retryAfterMs > 0) error.retryAfterMs = Math.min(retryAfterMs, 36e5);
    throw error;
  }
  return body;
}
function isAccountSession(session) {
  return Boolean(session?.access_token && UUID_PATTERN.test(String(session?.user?.id ?? "")) && session.user.is_anonymous !== true);
}
function betaAccountLinkErrorMessage(error) {
  if (error?.code === "ACCOUNT_LINK_RATE_LIMITED" || error?.status === 429) {
    const minutes = Math.max(1, Math.ceil((Number(error.retryAfterMs) || 6e4) / 6e4));
    return `게임 계정 연결 요청이 많아 잠시 제한되었습니다. 약 ${minutes}분 후 ‘게임 연결 다시 시도’를 눌러 주세요. 반복 로그인이나 새로고침은 필요하지 않습니다. (429)`;
  }
  return "로그인은 되었지만 게임 세션을 연결하지 못했습니다. 잠시 후 ‘게임 연결 다시 시도’를 눌러 주세요.";
}
function notifyAccountStatus(detail) {
  const frozen = Object.freeze({ ...detail });
  for (const listener of accountStatusListeners) {
    try {
      listener(frozen);
    } catch {
    }
...
function serializedAccountMutation(action) {
  const next = accountMutation.catch(() => null).then(action);
  accountMutation = next.catch(() => null);
  return next;
}
function getBetaAuthClient() {
  if (betaAuthClient) return betaAuthClient;
  betaAuthClient = createClient(BETA_SUPABASE_AUTH_URL, BETA_SUPABASE_PUBLISHABLE_KEY, {
    auth: {
      autoRefreshToken: true,
      detectSessionInUrl: true,
      persistSession: true
  ...
  return betaAuthClient;
}
function createBetaAccountSessionBridge({
  fetchImpl = globalThis.fetch?.bind(globalThis),
  getAuthSession = async () => {
    const { data, error } = await getBetaAuthClient().auth.getSession();
    if (error) throw error;
    return data.session || null;
  }
} = {}) {
  if (typeof fetchImpl !== "function") throw new TypeError("Beta account bridge requires fetch");
  let linkRetryAt = 0;
  async function ensurePlayerSession() {
    const response = await fetchImpl(PLAYER_SESSION_PATH, {
      method: "POST",
  ...
  async function linkSession(session, currentPlayer = null, options = {}) {
    if (!isAccountSession(session)) return null;
    const accountId = String(session.user.id).toLowerCase();
    if (options.force !== true && currentPlayer?.accountId === accountId && Number(currentPlayer.accountVerifiedUntilMs) > Date.now() + 6e4) {
      return options.returnCurrent === true ? { ok: true, protocolVersion: 2, player: currentPlayer } : null;
    }
    if (linkRetryAt > Date.now()) {
      const error = new Error("ACCOUNT_LINK_RATE_LIMITED");
      Object.assign(error, { code: "ACCOUNT_LINK_RATE_LIMITED", status: 429, retryAfterMs: linkRetryAt - Date.now() });
      throw error;
    }
    const unlinkPlayerAccount = async () => {
      const response = await fetchImpl(ACCOUNT_SESSION_PATH, {
        method: "DELETE",
        credentials: "same-origin",
        cache: "no-store",
        headers: { "content-type": "application/json" },
      ...
      if (body2.player.accountId !== null) throw new Error("BETA_ACCOUNT_UNLINK_MISMATCH");
      return body2.player;
    };
    const postAccountLink = async () => {
      ...
      try {
        ...
        if (error.status === 429) linkRetryAt = Date.now() + (error.retryAfterMs || 6e4);
        throw error;
      }
    };
    if (currentPlayer?.accountId && currentPlayer.accountId !== accountId) {
      currentPlayer = await unlinkPlayerAccount();
    }
    let body;
    try {
      body = await postAccountLink();
    } catch (error) {
      if (error?.code === "TEMP_PLAYER_SESSION_REQUIRED") {
        currentPlayer = (await ensurePlayerSession()).player;
        body = await postAccountLink();
      } else if (error?.code === "PLAYER_ACCOUNT_ALREADY_LINKED") {
        currentPlayer = await unlinkPlayerAccount();
        body = await postAccountLink();
      } else {
        throw error;
    ...
    if (body.player.playerId !== currentPlayer?.playerId) throw new Error("BETA_PLAYER_SESSION_MISMATCH");
    if (body.player.accountId !== accountId) throw new Error("BETA_ACCOUNT_SESSION_MISMATCH");
    notifyAccountStatus({ kind: "linked", accountId, playerId: body.player.playerId });
    return body;
  }
  return Object.freeze({
    async linkCurrent(currentPlayer = null) {
      return serializedAccountMutation(async () => {
        try {
          const session = await getAuthSession();
          if (!isAccountSession(session)) return null;
          if (!currentPlayer) currentPlayer = (await ensurePlayerSession()).player;
          return await linkSession(session, currentPlayer);
        } catch (error) {
          notifyAccountStatus({ kind: "error", code: String(error?.code || error?.message || "BETA_ACCOUNT_LINK_FAILED") });
          throw error;
        }
      });
    },
    async linkSession(session, currentPlayer = null, options = {}) {
      return serializedAccountMutation(async () => {
        try {
          if (!isAccountSession(session)) return null;
          if (!currentPlayer) currentPlayer = (await ensurePlayerSession()).player;
          return await linkSession(session, currentPlayer, options);
        } catch (error) {
          notifyAccountStatus({ kind: "error", code: String(error?.code || error?.message || "BETA_ACCOUNT_LINK_FAILED") });
          throw error;
        }
    ...
    async unlink() {
      return serializedAccountMutation(async () => {
        try {
          ...
          if (body.player.accountId !== null) throw new Error("BETA_ACCOUNT_UNLINK_MISMATCH");
          notifyAccountStatus({ kind: "unlinked", playerId: body.player.playerId });
          return body;
        } catch (error) {
          notifyAccountStatus({ kind: "error", code: String(error?.code || error?.message || "BETA_ACCOUNT_UNLINK_FAILED") });
          throw error;
        }
      });
    }
...
const liveAccountBridge = createBetaAccountSessionBridge();
function linkCurrentBetaAccountToPlayerSession(currentPlayer = null) {
  return liveAccountBridge.linkCurrent(currentPlayer);
}
function linkBetaAccountSession(session, currentPlayer = null, options = {}) {
  return liveAccountBridge.linkSession(session, currentPlayer, options);
}
function unlinkBetaAccountSession() {
  return liveAccountBridge.unlink();
}
export {
  linkCurrentBetaAccountToPlayerSession as a,
  betaAccountLinkErrorMessage as b,
  getBetaAuthClient as g,
  linkBetaAccountSession as l,
  unlinkBetaAccountSession as u
};
